...
var resizePizzas = function (size) {
  ...
  function changePizzaSizes(size) {
    ...
    var pizzaCollections = document.getElementsByClassName("randomPizzaContainer");

    var pizzas = Array.prototype.slice.apply(pizzaCollections);

    //calculates size difference
    var dx = determineDx(pizzas[0], size);

    //calculate offsetwidth
    var offWidth = pizzas[0].offsetWidth;

    //changes sizes of pizzas
    pizzas.forEach(function (pizza) {
      pizza.style.width = offWidth + dx + 'px';
    });
  }
...
function updatePositions() {
  ...
  var itemsCollection = document.querySelectorAll('.mover');
  var items = Array.prototype.slice.apply(itemsCollection);
